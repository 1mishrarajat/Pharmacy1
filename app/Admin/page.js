...
export default function AdminPage() {
  ...
  if (userRole === null) {
    // Skeleton loader block instead of plain "Loading..."
    return (
      <div className="flex flex-col items-center justify-center h-screen space-y-4 p-6">
        ...
        <div className="w-96 h-4 bg-gray-300 rounded-md animate-pulse"></div>
        <div className="w-72 h-4 bg-gray-300 rounded-md animate-pulse"></div>
        <div className="w-80 h-8 bg-gray-300 rounded-md animate-pulse mt-6"></div>
        <div className="grid grid-cols-2 gap-6 w-full max-w-xl mt-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-24 bg-gray-300 rounded-xl animate-pulse"></div>
          ))}
        </div>
      </div>
    );
          ...
          <main className="flex-1 p-6 sm:p-8 space-y-8">
            <div className="flex items-center justify-center h-64 sm:h-80 bg-gradient-to-br from-blue-200 via-teal-200 to-purple-200 p-4 sm:p-8 rounded-3xl shadow-lg">
              <div className="bg-white bg-opacity-80 rounded-3xl shadow-lg p-6 sm:p-12 text-center max-w-lg mx-auto">
                ...
                <p className="text-sm sm:text-lg text-gray-600 leading-relaxed">
                  Access various management tools to customize and control user data, subscriptions, and
                  settings.
                </p>
              </div>
            </div>
